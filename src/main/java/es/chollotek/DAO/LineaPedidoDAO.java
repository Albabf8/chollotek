...
public interface LineaPedidoDAO {
     ...
     *
     * 1) Funcionalidad Ajax de aumentar/disminuir cantidad:
     *    - Botón [+]: llama a este método con cantidad actual + 1
     *    - Botón [-]: llama a este método con cantidad actual - 1
     *      (si llegaría a 0, en vez de actualizar se elimina la línea)
     *
     * 2) Usuario añade producto que ya estaba en carrito:
     *    - Se suma la nueva cantidad a la existente
     *
     * IMPORTANTE:
     * - La cantidad DEBE ser >= 1 (si es 0 hay que eliminar la línea)
     * - Debe ejecutarse en la misma transacción que actualizar el importe del pedido
     *
     ...
     *
     * Se usa antes de añadir un producto al carrito para verificar si ya existe:
     * - Si existe: se actualiza su cantidad (suma la nueva cantidad a la existente)
     * - Si no existe: se inserta una nueva línea con cantidad inicial
     *
     * Esta lógica evita tener líneas duplicadas del mismo producto en un pedido.
     *
     ...
     *
     * Se usa cuando:
     * - El usuario pulsa "eliminar" en un producto del carrito
     * - La cantidad llegaría a 0 al disminuir (alternativa a actualizar a 0)
     *
     * Tras eliminar la línea, hay que recalcular y actualizar el importe
     * total del pedido en la misma transacción.
     *
     * Si era la última línea del pedido, el carrito quedaría vacío pero
     * el pedido seguiría existiendo (se puede eliminar o dejar para futuras compras).
     *
     ...
     *
     * 1) Usuario vacía el carrito por completo:
     *    - eliminarPorPedido(idPedido)  ← primero
     *    - PedidoDAO.eliminar(idPedido)  ← luego (opcional)
     *
     ...
     *
     * Se llama cuando el usuario añade un producto que NO estaba previamente
     * en su carrito. Antes de llamar a este método, siempre hay que verificar
     * con buscarLinea() que el producto no existe ya.
     *
     ...
     * @param con conexión activa (DEBE estar en transacción)
     * @throws Exception si hay error al insertar, violación FK, o datos inválidos
     */
     ...
     * - Mostrar el contenido del carrito con todos sus productos
     * - Generar el detalle de un pedido finalizado en el historial
     * - Calcular el importe total sumando (cantidad × precio) de cada línea
     *
     * Las líneas se devuelven en el orden en que fueron insertadas
     * (por idlinea), aunque la vista puede reordenarlas por nombre de producto.
     *
