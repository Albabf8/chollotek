    ...
    <body>
        ...
        <div class="container">
            ...
            <c:choose>
                <c:when test="${not empty sessionScope.usuario}">
                    <c:set var="listaCarrito" value="${lineasCarrito}" />
                </c:when>
                <c:otherwise>
                    <c:set var="listaCarrito" value="${sessionScope.carritoAnonimo}" />
                </c:otherwise>
            </c:choose>
                        ...
                        <table class="tabla-carrito">
                            ...
                            <tbody id="tablaCarritoBody">
                                ...
                                <c:forEach items="${listaCarrito}" var="linea">
                                    <c:set var="precio" value="${linea.producto.precio}" />
                                    <c:set var="subtotalLinea" value="${precio * linea.cantidad}" />
                                    ...
                                    <tr id="fila-${linea.idlinea}-${linea.idproducto}"
                                        ...
                                        <td>
                                            <div class="cantidad-controls" data-linea="${linea.idlinea}">
                                                <button type="button"
                                                        class="btn-cantidad"
        ...
        <script>
                                                ...
                                                function restarCantidadAjax(idLinea, cantidadActual, idProducto) {
                                                    // Comprobación previa: no permitir decrementar si la cantidad es 1
                                                    if (cantidadActual <= 1) {
                                                        //mostrarMensaje('La cantidad mínima es 1', 'info');
                                                        return; // Salir sin hacer la petición AJAX
                                                    }
                                                    ...
                                                    fetch(contextPath + '/AjaxController?accion=restarCantidad'
                                                            ...
                                                            .then(function (data) {
                                                                if (data.exito) {
                                                                    // Ya no se maneja data.eliminado, porque nunca se elimina
                                                                    var nuevaCantidad = data.nuevaCantidad;
                                                                    var key = idLinea + '-' + idProducto;
                                                                    document.getElementById('cantidad-' + key).textContent = nuevaCantidad;
                                                                    actualizarSubtotalLinea(key, nuevaCantidad);
                                                ...
                                                function actualizarBotones(idLinea, nuevaCantidad, idProducto) {
                                                    ...
                                                    if (!fila)
                                                        return;
                                                    var botones = fila.querySelectorAll('.btn-cantidad');
                                                    if (botones.length >= 2) {
